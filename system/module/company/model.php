    ...
    public function getContact()
    {
        ...
        foreach($contact as $item => $value)
        {
            if($value)
            {
                ...
                else if($item == 'envelope') //email -> envelope
                {
                    $value = str2Entity($value);
                    $contact->envelope = html::mailto($value, $value);
                }
                else if($item == 'weibo')
                {
                    $contact->weibo = html::a("http://weibo.com/{$value}", $value, "target='_blank'");
                }
                else if($item == 'internet_explorer')
                {
                    if($_SERVER['HTTP_HOST'] != $value) $contact->internet_explorer = html::a("http://{$value}", $value, "target='_blank'");
                    if($_SERVER['HTTP_HOST'] == $value) unset($contact->$item);
                }
                else if($item == 'comments')
                {
                    $contact->comments = html::a("http://www.taobao.com/webww/ww.php?ver=3&touid={$value}&siteid=cntaobao&status=2&charset=utf-8", $value, "target='_blank'");
                }
                else if($item == 'phone')
                {
